...
class ProxyChecker:
    ...
    def get_my_ip(self):
        sites = ['http://ipinfo.io/ip', 'https://api.ipify.org/', 'http://ifconfig.io/ip', 'https://www.trackip.net/ip', 'https://ip.seeip.org/ip']
    ...
    def get_info(self, url=None, proxy=None):
        ...
        if url != None:
            ...
            for protocol in ['http', 'socks4', 'socks5']:
                ...
                try:
                    start = time()
                    response = requests.get(random.choice(judges), proxies=proxy_dict, headers=headers, timeout=3)
                    finish = time() - start
